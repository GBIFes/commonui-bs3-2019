...
(function ($) {
  ...
  $.i18n.properties = function (settings) {
    ...
    // Ensure an array
    const files = (settings.name && settings.name.constructor === Array) ? settings.name : [settings.name];
    ...
    files.forEach((file) => {
      let defaultFileName; let shortFileName; let longFileName; let
        fileNames;
      // 1. load base (eg, Messages.properties)
      defaultFileName = `${settings.path + file}.properties`;
      // 2. with language code (eg, Messages_pt.properties)
      const shortCode = settings.language.substring(0, 2);
      shortFileName = `${settings.path + file}_${shortCode}.properties`;
      // 3. with language code and country code (eg, Messages_pt_BR.properties)
      if (settings.language.length >= 5) {
        const longCode = settings.language.substring(0, 5);
  ...
  $.i18n.prop = function (key /* Add parameters as function arguments as necessary  */) {
    ...
    let value = (namespace) ? $.i18n.map[namespace][key] : $.i18n.map[key];
    if (value === null) {
  ...
  function parseData(data, settings) {
    ...
    for (let i = 0, j = lines.length; i < j; i++) {
      ...
      if (line.length > 0 && line.match('^#') != '#') { // skip comments
        ...
        if (pair.length > 0) {
          /** Process key & value */
          const name = decodeURI(pair[0]).trim();
          ...
          if (settings.mode == 'vars' || settings.mode == 'both') {
            ...
            if (regPlaceHolder.test(value)) {
              const parts = value.split(regPlaceHolder);
              ...
              const fnExpr = `"${value.replace(regRepPlaceHolder, '"+v$1+"')}"`;
  ...
  function unescapeUnicode(str) {
    ...
    return codes.reduce((acc, val) => acc + String.fromCharCode(val), '');
  }
}(jQuery));
